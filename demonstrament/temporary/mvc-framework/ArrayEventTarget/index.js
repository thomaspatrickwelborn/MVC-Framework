export default class ArrayEventTarget extends Array {
  constructor() {
    super(...arguments)
    Object.defineProperties(this, {
      addEventListener: {
        ...
        enumerable: false,
        writable: false,
        value: function addEventListener($eventName, $eventCallback) {
          if(this.#eventListenerArgumentsValid(
            $eventName, $eventCallback
          )) return
          let eventCallbackName = this.#eventCallbackNameFromEventCallback($eventCallback)
      ...
      removeEventListener: {
        ...
        enumerable: false,
        writable: false,
        value: function removeEventListener($eventName, $eventCallback) {
          if(this.#eventListenerArgumentsValid(
            $eventName, $eventCallback
          )) return
          let eventCallbackName = this.#eventCallbackNameFromEventCallback($eventCallback)
  ...
  #eventCallbackNameFromEventCallback($eventCallback) {
    return (
      $eventCallback.name.length === 0
    ) ? 'anonymous'
      : $eventCallback.name
  }
  #eventListenerArgumentsValid($eventName, $eventCallback) {
    return (
