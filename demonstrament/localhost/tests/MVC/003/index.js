import { FetchRouter, Model, View, Control } from '/dependencies/mvc-framework.js'
    ...
    views: { photosView: new View({
      ...
      events: {
        ...
        'selectors.photosControlButton click': function photosControlButtonClick($event) {
          ...
          photosControl.models.photosModel.content.assign({ currentIndex: crement })
        },
      },
      templates: {
        // Image Template
    ...
    models: { photosModel: new Model({
      ...
      events: {
        'content push': function photosPush($event) {
          photosControl.views.photosView.renderElement({
            name: 'default', 
            data: photosControl.models.photosModel.content,
          })
        },
        'setCurrentIndex': function setCurrentIndex($event) {
          const { currentIndex } = $event.detail
          let photoIndex = 0
          for(const $photo of photosControl.views.photosView.selectors.photo) {
      ...
      fetch: { photosRouter: new FetchRouter({
        ...
        events: {
          'routes.photos status': async function photosStatus($event) {
            ...
            photosControl.models.photosModel.content.push(...photos)
          },
        },
      }, { enableEvents: true }) },
    },
